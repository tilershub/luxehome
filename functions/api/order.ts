  ...
  const itemRows = o.items.map(item => `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #e8e2da;">
        <div style="font-weight:600;color:#1e1a17;font-size:14px;">${item.name}</div>
        <div style="color:#8c7b6e;font-size:12px;margin-top:3px;">${item.desc}</div>
      </td>
      <td style="padding:10px 0;border-bottom:1px solid #e8e2da;text-align:right;font-weight:600;color:#1e1a17;font-size:14px;white-space:nowrap;vertical-align:top;">
        ${fmt(item.price)}
      </td>
    </tr>
  `).join('');
  ...
  const scheduleRows = o.paySchedule.map((p, i) => `
    <tr style="${i === 0 ? 'background:#fdf6f0;' : ''}">
      <td style="padding:10px 14px;border-bottom:1px solid #e8e2da;">
        <div style="font-weight:${i === 0 ? '700' : '500'};color:${i === 0 ? '#c4784a' : '#1e1a17'};font-size:13px;">
          ${p.pct}% — ${p.label}${i === 0 ? '&nbsp; <span style="background:#c4784a;color:#fff;font-size:10px;padding:2px 7px;border-radius:20px;letter-spacing:.06em;font-weight:700;">PAY NOW</span>' : ''}
        </div>
        <div style="color:#8c7b6e;font-size:11px;margin-top:3px;">${p.desc}</div>
      </td>
      <td style="padding:10px 14px;border-bottom:1px solid #e8e2da;text-align:right;font-weight:${i === 0 ? '700' : '600'};color:${i === 0 ? '#c4784a' : '#1e1a17'};font-size:13px;white-space:nowrap;vertical-align:top;">
        ${fmt(p.amount)}
      </td>
    </tr>
  `).join('');
...
<head>
  ...
  <title>TILERSHUB Invoice — ${o.orderRef}</title>
</head>
<body style="margin:0;padding:20px 0;background:#f0ebe3;font-family:Arial,Helvetica,sans-serif;">
  <div style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:6px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
    ...
    <!-- Order ref bar -->
    <div style="background:#c4784a;padding:11px 40px;display:flex;justify-content:space-between;align-items:center;">
    ...
    <div style="padding:36px 40px;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;border:1px solid #e8e2da;border-radius:4px;overflow:hidden;">
        ...
        <tr>
          ...
          <td style="padding:8px 16px;font-size:13px;color:#1e1a17;font-weight:500;border-bottom:1px solid #f0ebe3;">${o.phone}</td>
        </tr>
        <tr>
          <td style="padding:8px 16px;font-size:12px;color:#8c7b6e;width:130px;">Location</td>
          <td style="padding:8px 16px;font-size:13px;color:#1e1a17;font-weight:500;">${o.location}</td>
        </tr>
      </table>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:6px;">
        ${itemRows}
        <tr>
          <td style="padding:14px 0 0;font-size:14px;font-weight:700;color:#1e1a17;">Estimated Total</td>
          <td style="padding:14px 0 0;text-align:right;font-size:20px;font-weight:700;color:#c4784a;">${fmt(o.total)}</td>
        </tr>
      </table>
      ...
      <!-- Bank Transfer -->
      <div style="background:#1e1a17;border-radius:6px;padding:26px;margin-bottom:28px;">
        <div style="font-size:10px;font-weight:700;letter-spacing:.13em;text-transform:uppercase;color:#c4784a;margin-bottom:16px;">Bank Transfer Details</div>
        <table width="100%" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td style="padding:6px 0;font-size:13px;color:#f5f0e8;font-weight:600;">TILERSHUB (Pvt) Ltd</td>
          </tr>
          <tr>
        ...
        <div style="margin-top:16px;padding-top:16px;border-top:1px solid rgba(255,255,255,.07);font-size:12px;color:rgba(245,240,232,.55);line-height:1.7;">
          ...
          <strong style="color:#c4784a;">tilershub@gmail.com</strong>
          or WhatsApp <strong style="color:#c4784a;">+94 774 503 744</strong>
          with your order reference <strong style="color:#f5f0e8;">${o.orderRef}</strong>.
        </div>
      </div>

      <!-- Next Steps -->
      <div style="background:#f5f0e8;border-left:3px solid #c4784a;padding:18px 22px;border-radius:0 4px 4px 0;margin-bottom:8px;">
        <div style="font-size:12px;font-weight:700;color:#1e1a17;margin-bottom:8px;">What happens next?</div>
        <ol style="margin:0;padding-left:18px;font-size:13px;color:#5c5047;line-height:2;">
          ...
          <li>Send payment proof to <strong>tilershub@gmail.com</strong> or WhatsApp +94 774 503 744</li>
    ...
    <div style="background:#f5f0e8;padding:24px 40px;border-top:1px solid #e8e2da;text-align:center;">
      <div style="font-family:Georgia,serif;font-size:16px;font-weight:700;color:#1e1a17;margin-bottom:5px;">TILERSHUB</div>
      <div style="font-size:12px;color:#8c7b6e;">tilershub.lk &nbsp;·&nbsp; +94 774 503 744 &nbsp;·&nbsp; tilershub@gmail.com</div>
...
function buildAdminEmail(o: OrderPayload): string {
  const itemRows = o.items.map(item => `
    <tr>
      ...
      <td style="padding:9px 12px;border-bottom:1px solid #2e2e2e;font-size:13px;color:#c4784a;text-align:right;font-weight:600;white-space:nowrap;">${fmt(item.price)}</td>
    </tr>
  `).join('');
...
<body style="margin:0;padding:20px 0;background:#111;font-family:Arial,Helvetica,sans-serif;">
  <div style="max-width:600px;margin:0 auto;background:#1e1e1e;border-radius:6px;overflow:hidden;">
    ...
    <div style="padding:28px 32px;">
      ...
      <div style="margin-bottom:24px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #2e2e2e;border-radius:4px;overflow:hidden;">
          ...
          <tr>
            ...
            <td style="padding:8px 14px;font-size:13px;color:#c4784a;border-bottom:1px solid #2e2e2e;">${o.email}</td>
          </tr>
          <tr style="background:#252525;">
      ...
      <div style="margin-bottom:24px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #2e2e2e;border-radius:4px;overflow:hidden;">
          ...
          <tr style="background:#252525;">
            ...
            <td style="padding:12px 14px;text-align:right;font-size:20px;font-weight:700;color:#c4784a;white-space:nowrap;">${fmt(o.total)}</td>
          </tr>
        </table>
      </div>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #2e2e2e;border-radius:4px;overflow:hidden;">
        ${o.paySchedule.map((p, i) => `
          <tr style="${i === 0 ? 'background:#1e1a17;' : i % 2 === 0 ? 'background:#252525;' : ''}">
            <td style="padding:9px 14px;font-size:12px;color:${i === 0 ? '#c4784a' : '#888'};border-bottom:1px solid #2e2e2e;">${p.pct}% — ${p.label}</td>
            <td style="padding:9px 14px;font-size:12px;color:#666;border-bottom:1px solid #2e2e2e;">${p.desc}</td>
            <td style="padding:9px 14px;font-size:13px;font-weight:600;color:${i === 0 ? '#c4784a' : '#e0e0e0'};text-align:right;border-bottom:1px solid #2e2e2e;white-space:nowrap;">${fmt(p.amount)}</td>
          </tr>
        `).join('')}
      </table>
    ...
    <div style="padding:16px 32px;border-top:1px solid #2e2e2e;font-size:11px;color:#555;text-align:center;">
      TILERSHUB &nbsp;·&nbsp; tilershub.lk &nbsp;·&nbsp; Auto-generated order notification
    </div>
